...
public class PaymentService {
     ...
     * </ol>
     *
     * @return true nếu đây là lần xử lý đầu tiên và thanh toán thành công
     */
    public boolean handleCallback(GatewayCallback callback) {
        ...
        return Tx.write(con -> {
            ...
            if (order != null && order.isOnline()) {
                orderService.confirmOnlineAfterPaid(con, order, now);
            }
            return true;
        });
    }
     ...
     */
    public void refund(int orderId, int actorId) {
        LocalDateTime now = DateTimeUtil.now();
        Tx.writeVoid(con -> {
            Order order = orderDAO.findById(con, orderId);
            if (order == null) {
            ...
            if (OrderStatus.COMPLETED.name().equals(order.getOrderStatus())) {
                throw new BusinessException("Đơn đã giao cho khách, không hoàn tiền qua màn hình này được.");
            }
