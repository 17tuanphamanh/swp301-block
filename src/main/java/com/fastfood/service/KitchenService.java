...
public class KitchenService {
    ...
    public Page<OrderItem> recentReady(int pageNo) {
        int page = Page.safePage(pageNo);
        int offset = Page.offset(page, Page.SIZE);
        return Tx.read(con -> new Page<>(
                orderItemDAO.findReadyPage(con, offset, Page.SIZE),
                page, Page.SIZE,
                orderItemDAO.countReady(con)));
    }
     ...
     * dòng này rồi mới đếm số món đã vào bếp (xem {@link OrderService#cancelByCustomer}).
    ...
    public void claim(int orderItemId, int userId) {
        ...
        Tx.writeVoid(con -> {
            ...
            orderService.lockOrder(con, item.getOrderId());
     ...
     * nên nó thuộc về thu ngân — xem {@link OrderService#cancelByStaff}.
     */
