...
└── src/main/
    ...
    │   ├── model/entity/      27  26 lớp ánh xạ đúng 26 bảng
    │   ├── model/dto/         10  Dữ liệu đã gộp sẵn cho tầng hiển thị
    │   ├── dao/               30  23 lớp truy vấn chia theo vai trò + JdbcSupport, xem §2.2
    │   ├── service/           33  25 lớp nghiệp vụ chia theo vai trò + Tx, xem §2.1
    ...
    │   └── controller/        38  31 servlet chia theo vai trò
...
### 3.5 Hộp thông báo — nơi khách đọc được bốn tin của một đơn

`NotificationService` sinh tin cho bốn sự kiện: đơn được xác nhận, món sẵn sàng, đơn bị huỷ,
đơn hết hiệu lực. Tin đi qua `NotificationSender`, mà bản chạy thử là lớp giả lập — **không có
bức thư nào thật sự tới hộp thư của khách.** Vì vậy `/notifications` là nơi duy nhất họ đọc
được những tin đó, và nó không phải một tiện ích thêm vào cho đẹp: tình huống ở §3.4 kết thúc
bằng một khoản tiền được hoàn lại, mà nếu khách đã đóng trang thanh toán thì cả câu chuyện chỉ
còn nằm ở đây.
...
vừa là dấu vết của việc gửi tin, vừa là **hộp thông báo** khách đọc ở `/notifications` — xem §3.5.
...
| `/notifications` | NotificationServlet | customer/notifications.jsp — **hộp thông báo**, xem §3.5 |
| `/profile` | ProfileServlet | customer/profile.jsp |

Cộng hai trang công khai `/menu` và `/product/detail` là **7 màn hình** khách hàng đi qua.
...
| Nhật ký đăng nhập | `LOGIN_SUCCESS` · `LOGIN_FAILED` · `LOGIN_BLOCKED` · `LOGOUT` · `PASSWORD_RESET_*`, ghi bằng **giao dịch riêng** | Ghi cùng giao dịch với thao tác bị từ chối là tự xoá bằng chứng: giao dịch huỷ thì dòng nhật ký đáng chú ý nhất huỷ theo |
...
|---|---|---|---|
| Không chạm cơ sở dữ liệu | `*Test` | Mọi máy, không cần gì thêm | 168 |
| Chạy thật xuống cơ sở dữ liệu | `*IT` | Cần SQL Server ở `localhost:1433` | 336 |

Nhóm đầu gồm 162 bài logic thuần (`BusinessMathTest`, `PickupCodeGeneratorTest`,
`OrderStateTest`, `PasswordPolicyTest`, `SafeRedirectTest`, `LoginThrottleTest`,
`CsrfTokenTest`, và bốn bài về bộ lọc: `RoleAuthorizationFilterTest`, `CsrfFilterTest`,
`RequestPathTest`, `RoutePolicyTest`) và 6 bài dựng
